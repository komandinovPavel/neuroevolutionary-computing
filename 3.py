...
class NeuralNetwork:
    ...
    def forward(self, inputs):
        # Переход на LeakyReLU
        hidden_input = np.dot(inputs, self.weights_in)
        hidden = np.where(hidden_input > 0, hidden_input, hidden_input * 0.01)  # LeakyReLU с alpha=0.01
...
class Neuron:
    def __init__(self, input_size, output_size, bits=16):
        self.bits = bits
        # Инициализация весов в диапазоне [0, 2^bits/2], чтобы после декодирования они были ближе к [-0.5, 0.5]
...
class SANE:
    def __init__(self, pop_size, input_size, hidden_size, output_size, blueprint_size=50):
        ...
        self.STAGNATION_THRESHOLD = 300  # Увеличиваем порог стагнации
        self.epoch = 0
    ...
    def evaluate(self, env, max_steps=500):
        ...
        # Обновляем приспособленность нейронов (средняя по 5 лучшим И НС)
        ...
        for neuron in self.neurons:
            for i in range(len(neuron.weights_in)):
                for bit in range(neuron.bits):
                    if np.random.rand() < 0.001:  # 0.1% на бит  # 0.1% на бит
        ...
        for blueprint in self.blueprints:
            for i in range(self.hidden_size):
                if np.random.rand() < 0.01:  # 1% вероятность
                    ...
                    blueprint[i] = new_idx
                elif np.random.rand() < 0.1:  # Снижаем с 50% до 10%
...
def plot_network(network, filename="network.png"):
    ...
    pos = nx.multipartite_layout(G, subset_key="layer", subset_order=["input", "hidden", "output"])
