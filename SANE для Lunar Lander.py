...
class Neuron:
    def __init__(self, input_size, output_size, num_connections=3):
        self.label = np.random.randint(0, 256)  # 8-битная метка (lec8.pdf)
        ...
        self.weights_in = np.random.randn(num_connections) * 0.1
        self.weights_out = np.random.randn(num_connections) * 0.1
        self.fitness = 0.0
        self.usage_count = 0
...
# Класс SANE
class SANE:
    def __init__(self, pop_size, input_size, hidden_size, output_size, blueprint_size=80):
        ...
        self.network_cache = {}  # Кэш для сетей
    ...
    def run_episode(self, env, network, max_steps=500):
    ...
    def evaluate(self, env, max_steps=500):
        ...
        avg_fitness = np.mean([b.fitness for b in self.blueprints])
    ...
    def mutate_neurons(self):
        for neuron in self.neurons:
            if np.random.rand() < 0.001:  # 0.1% вероятность
    ...
    def mutate_blueprints(self):
        for blueprint in self.blueprints[1:]:  # Пропуск элиты
            if np.random.rand() < 0.01:  # 1% вероятность
                ...
                if np.random.rand() < 0.5:  # 50% на потомка
                    sorted_neurons = sorted(self.neurons, key=lambda x: x.fitness, reverse=True)
